...
# -----------------------------------------
# SCOPE.md is the only rule here whose cost is UNRECOVERABLE. It says so itself:
...
#
# And SCOPE.md names its own failure mode exactly:
#
...
#   - `heltec`: SCOPE.md's instrument carve-out is explicit -- instruments/ holds
...
#   - silicon part numbers such as `rp2350`: DISTRIBUTION.md promises a declared
#     target set, and docs/HARDWARE-BACKENDS.md surveys crypto accelerators.
#     Naming a chip is not naming a product's architecture.
#
# The distinction throughout is SCOPE.md's own: a fact about the protocol or the
...
# to hold these words.
PATHSPEC=(':(exclude)tools/check_scope.sh')
...
if [ "$rc" = 0 ]; then
    ...
    bad "$hits forbidden name(s) present. SCOPE.md: this repository is OPEN and"
