...
#   third_party/       pinned upstream source under its own terms. Stamping our
#                      header onto someone else's file would be a false claim
#                      about its provenance -- the opposite of what this checks.
#                      Their licences are recorded in DEPS.md and NOTICE.
#
# USAGE
#   tools/check_spdx.sh            scan tracked files
#   tools/check_spdx.sh --staged   scan only staged files (pre-commit)
#
set -euo pipefail
...
while IFS= read -r f; do
    ...
    # tools/check_cleanroom.sh's quoted `SPDX-License-Identifier: *GPL` pattern,
