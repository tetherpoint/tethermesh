...
# ── 3. pinned commits in DEPS.md must be the commits actually checked out ──
# DEPS.md is the provenance record: "a compatibility result that cannot name
...
if [ -f DEPS.md ] && [ -f .gitmodules ]; then
    while read -r sha path _; do
        [ -n "$path" ] || continue
        sha="${sha#[-+U]}"          # strip submodule status markers
        checked=$((checked+1))
        if ! grep -q "$sha" DEPS.md; then
            short="${sha:0:12}"
            fail "submodule '$path' is at $sha, which DEPS.md does not record.
              DEPS.md is the provenance record for every measurement in this
...
done < <(grep -rhoE '\btools/[a-z0-9_]+\.sh' --include='*.md' . 2>/dev/null | sort -u)
