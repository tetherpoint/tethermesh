...
#     docs/DEPS.md; a copy here is a GPL file in our repo)
#   - running a code generator over their .proto — the output is derived from
#     a GPL input. This is why docs/PLAN.md specifies a hand-written ~300-line
...
#
# USAGE
#   gates/check_cleanroom.sh            scan the tree
#   gates/check_cleanroom.sh --staged   scan only staged files (pre-commit)
#
set -euo pipefail
...
# strings to gates/gpl-patterns.txt shrinks the unavoidable exemption from a
# whole program to a list of literals anyone can read in seconds.
PATTERN_FILE="$ROOT/gates/gpl-patterns.txt"
...
while IFS= read -r f; do
    ...
    case "$f" in
        ...
        gates/gpl-patterns.txt) continue ;;
        # docs/EXTENSIONS.md joined this list on 2026-08-16, when the licence
        ...
        # NOTICE joined on 2026-08-16 for the same reason as docs/EXTENSIONS.md:
        ...
        # holds the GPL header strings, which moved to gates/gpl-patterns.txt.
        # It needs this and only this.
        docs/PLAN.md|README.md|NOTICE|docs/EXTENSIONS.md|docs/*|docs/WIRE_REFERENCE.md|gates/check_cleanroom.sh|tests/instrument/*) may_name_it=1 ;;
        # Pinned upstream code, licence recorded in docs/DEPS.md. Not held to rules
        ...
        code/third_party/*) may_name_it=1 ;;
    esac

    case "$f" in
        *.proto)
            fail "$f — vendored .proto. Upstream protos are GPL-3.0; read them
              at the pinned commit in docs/DEPS.md, never copy them into this tree." ;;
        *.pb.c|*.pb.h|*_pb2.py)
            fail "$f — generated protobuf output. Derived from a GPL-3.0 .proto.
              docs/PLAN.md specifies a hand-written codec for exactly this reason." ;;
    esac
...
for d in "$ROOT"/code/third_party/*/; do
    [ -d "$d" ] || continue
    while IFS= read -r hit; do
        [ -n "$hit" ] || continue
        fail "${hit#"$ROOT"/} — GPL licence header inside a vendored submodule.
              docs/DEPS.md records these as permissively licensed; a version bump
...
while IFS= read -r f; do
    ...
    p="$ROOT/$f"
    [ -f "$p" ] || continue
    case "$f" in gates/check_cleanroom.sh|gates/fetch_oracle.sh|*.md) continue ;; esac
