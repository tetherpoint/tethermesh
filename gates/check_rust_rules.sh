...
#
# check_rust_rules.sh — enforce the crate-level rules docs/DISTRIBUTION.md commits to.
#
# WHY THESE ARE CHECKED AND NOT TRUSTED
# -------------------------------------
# docs/DISTRIBUTION.md promises three properties that are only worth promising if
...
#
# USAGE
#   gates/check_rust_rules.sh            source-level checks
#   gates/check_rust_rules.sh --binary <obj.o|lib.a>  also check for panic machinery
...
# code/third_party/ holds pinned upstream code, and OUR source rules do not govern
...
rs_files=$(find "$ROOT" -name '*.rs' -not -path '*/target/*' -not -path '*/code/third_party/*' 2>/dev/null || true)
...
# In --binary mode the SOURCE pass is skipped: gates/check_all.sh runs it once
...
"
    ...
    if [ -n "$alloc_syms" ]; then
        fail "$LIBA LINKS A RUST ALLOCATOR. docs/DISTRIBUTION.md forbids one outright:
