...
# docs/DISTRIBUTION.md calls the only thing that makes shipping a crypto binary
...
: > "$MANIFEST"
{
    ...
    echo
    # docs/DISTRIBUTION.md requires this caveat in THREE places -- the release notes,
...
while read -r target crate ceiling measured note; do
    ...
    "$ROOT/gates/check_rust_rules.sh" --binary "$src" \
        || die "$target: the archive does not satisfy the crate rules docs/DISTRIBUTION.md promises — it will not be released"
    ...
    cp "$ROOT/code/c-api/include/tethermesh.h" "$OUT/$HDR"
