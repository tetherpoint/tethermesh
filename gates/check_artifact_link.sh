...
# -----------------------------------------------------------------------
# docs/DISTRIBUTION.md promises: "CI builds a minimal C consumer against each
...
#
# USAGE
#   gates/check_artifact_link.sh <target> <path-to-cross-gcc-prefix>
#
#   e.g. gates/check_artifact_link.sh thumbv8m.main-none-eabihf \
...
cat > "$WORK/code/c-api/Cargo.toml" <<EOF
[package]
name = "tmffi"
version = "0.0.0"
edition = "2021"
...
opt-level = "z"
EOF
cat > "$WORK/code/c-api/src/lib.rs" <<'EOF'
#![no_std]
#[panic_handler]
...
A="$WORK/code/c-api/target/$TARGET/release/libtmffi.a"
