...
#
# T8 -- regenerate code/c-api/include/tethermesh.h from the crate.
#
...
# gates/check_header.sh fails if the committed file and the crate disagree.
set -euo pipefail
...
"$CBINDGEN" --config code/c-api/cbindgen.toml --crate tmffi ffi > "$out.new"
...
for leak in 'PacketHistory' 'Outbox<' 'uintptr_t' 'TmKey' 'TmCtx'; do
    if grep -q "$leak" "$out.new"; then
        echo "REFUSING: generated header leaks '$leak' -- check code/c-api/cbindgen.toml" >&2
        rm -f "$out.new"
        exit 1
    fi
done
