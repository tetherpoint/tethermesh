...
#
# docs/DISTRIBUTION.md: "header struct layouts are frozen within a major version, and
...
HEADER=code/c-api/include/tethermesh.h
BASE=code/c-api/abi-baseline.txt
...
if [ "${1:-}" = "--accept" ]; then
    {
        echo "# SPDX-FileCopyrightText: 2026 Matthew Klapman"
...
if [ ! -f "$BASE" ]; then
    ...
    warn "Create one with: gates/check_abi_stability.sh --accept"
    exit 3
fi
...
if [ "$cur_ver" -gt "$base_ver" ]; then
    ...
    say "That is a declared break. Refresh with: gates/check_abi_stability.sh --accept"
    exit 0
fi
