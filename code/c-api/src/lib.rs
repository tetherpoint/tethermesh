...
//! one. `gates/check_artifact_link.sh` in that repository proves the shape
...
// `docs/DISTRIBUTION.md` calls the FFI boundary the trust edge for this reason: "a
...
///
/// **The header is generated now** (`gates/generate_header.sh`, gated by
/// `gates/check_header.sh`), so the drift this was built to mitigate is largely
...
///
/// **This function is the mechanism enforcing `docs/PLAN.md`'s shared-airtime rule,**
...
///
/// `docs/PLAN.md` fixes upstream's retry behaviour as a **ceiling, never a target**.
...
/// because it has neither a clock nor a radio -- `docs/DISTRIBUTION.md` puts
...
/// `docs/WIRE_REFERENCE.md` from upstream's own documentation. Only the
...
/// `docs/WIRE_REFERENCE.md` records `PRIVATE_APP = 256` with upstream's
...
//
// `docs/PLAN.md` fixes the rule that upstream's retry behaviour is a CEILING and
...
/// returned slice. That is the FFI trust edge `docs/DISTRIBUTION.md` names: a caller
