...
//! mechanism enforcing `docs/PLAN.md`'s shared-airtime rule. Sampling it is not the
//! same as settling it.
//!
//! # What is deliberately not proven
//!
//! Nothing here says anything about pointer validity. The FFI boundary is the
//! trust edge `docs/DISTRIBUTION.md` names: a caller passing a bad pointer or a
