...
Source implementations that require particular hardware — a GNSS driver, a ranging driver — are out of scope here, as is any radio driver. This repository specifies the protocol and provides a portable implementation of it. See `docs/SCOPE.md`.

## Bundles, and what each one needs before it can be written

The suite is **one crate per bundle**, so a consumer takes the core plus only what they want. `docs/SCOPE.md` records how a bundle crate is added and gated; this is what each one *depends on*.
...
**Matthew Klapman will not assert any patent they hold or control
...
`gates/check_cleanroom.sh` enforces both on every run. An
